...
public class Box : MonoBehaviour
{
            ...
            else if (items.Count <= 0)
            {
                ...
                if (stateNum == 0)
                {
                    if (collision.gameObject.GetComponent<Mario>().marioMode != 0)
                    {
                        ...
                        {
                        Debug.Log("flag2");
                            GetComponent<PhotonView>().RPC("RPC_BoxDestroy", RpcTarget.All);
                        }
                    }
    ...
    void RPC_BoxDestroy()
    {
        ...
        Destroy(gameObject);
        // 4조각 brick 없애기
        //var a = PhotonNetwork.Instantiate("Prefabs/BrokenBrick", transform.position, Quaternion.identity);
        var a = Instantiate(brokenBrick, transform.position, Quaternion.identity);
        Destroy(a, 0.5f);
    }
