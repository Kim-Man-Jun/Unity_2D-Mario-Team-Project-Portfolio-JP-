...
public class Room : MonoBehaviourPunCallbacks
{
	...
	IEnumerator Cor_Scroll_MapSelect()
	{
		...
		//var b = lobby_obj.GetComponent<Lobby>().real_object_list.ToList<GameObject>();
		//for (int i = 0; i < b.Count; i++)
		//{
		//	b[i].SetActive(false);
		//}
		//b[currentMapNum-1].SetActive(true);
	...
	public void OnClick_Ready()
	{
		...
		{
			Debug.Log(lobby_obj.GetComponent<Lobby>().real_json_list[currentMapNum - 1] + " 방장이 프로퍼티 설정함" + currentMapNum + " " );
			// {"room_state", true} means GameStart
			PhotonNetwork.CurrentRoom.SetCustomProperties(new Hashtable() { { "room_state", true }, { "map_num", lobby_obj.GetComponent<Lobby>().real_json_list[currentMapNum - 1] } });
			WIndowManager.instance.mapName = lobby_obj.GetComponent<Lobby>().real_json_list[currentMapNum - 1];
